...
kproto.default <-
  function(x,
    ...
    if (ncol(x) < 2)
      stop("For clustering x should contain at least two variables!")
    if (iter.max < 1 |
        nstart < 1)
      stop("iter.max and nstart must not be specified < 1!")
    if (!is.null(lambda)) {
      if (any(lambda < 0))
        stop("lambda must be specified >= 0!")
      if (!any(lambda > 0))
        stop("lambda must be specified > 0 for at least one variable!")
    }
    ...
    if (length(lambda) > 1) {
      if (length(lambda) != sum(c(numvars, catvars)))
        stop(
          "If lambda is a vector, its length should be the sum of numeric and factor variables in the data frame!"
        )
    }
    if (is.null(lambda)) {
      if (anynum & anyfact) {
        ...
        vcat <-
          mean(sapply(x[, catvars, drop = FALSE], function(z)
            return(1 - sum((table(z) / length(z)) ^ 2
            ))))
        if (vnum == 0) {
    ...
    initproto <- function() {
      if (!is.data.frame(k)) {
        ...
        if (length(k) > 1) {
          ...
          if (length(unique(ids)) != length(ids))
            stop("If k is specified as a vector it should contain different indices!")
          if (any(ids < 1) |
              any(ids > nrow(x)))
            stop("If k is specified as a vector all elements must be valid indices of x!")
          #check for integer
          protos <- x[ids, ]
        }
        rm(ids)
      ...
      if (is.data.frame(k)) {
        if (length(names(k)) != length(names(x)))
          stop("k and x have different numbers of coloumns!")
        if (any(names(k) != names(x)))
          stop("k and x have different coloumn names!")
        if (anynum) {
          if (any(sapply(k, is.numeric) != numvars))
            stop("Numeric variables of k and x do not match!")
        }
        if (anyfact) {
          if (any(sapply(k, is.factor) != catvars))
            stop("Factor variables of k and x do not match!")
        }
        protos <- k
        k <- nrow(protos)
      }
    ...
    clustf <- function() {
      ...
      if (k == 1) {
        ...
        protos[1, catvars] <-
          sapply(x[, catvars, drop = FALSE], function(z)
            levels(z)[which.max(table(z))])
        
        nrows <- nrow(x)
        dists <-
          matrix(NA, nrow = nrows, ncol = k) #dists <- rep(NA, nrows)
        d1 <-
          (x[, numvars, drop = FALSE] - matrix(
            rep(as.numeric(protos[1, numvars, drop = FALSE]), nrows),
            nrow = nrows,
            byrow = T
          )) ^ 2
        if (length(lambda) == 1)
          d1 <- rowSums(d1)
        if (length(lambda) > 1)
          d1 <- d1 %*% lambda[numvars]
        d2 <-
          sapply(which(catvars), function(j)
            return(x[, j] != rep(protos[1, j], nrows)))
        if (length(lambda) == 1)
          d2 <- lambda * rowSums(d2)
        if (length(lambda) > 1)
          d2 <- d2 %*% lambda[catvars]
        dists[, 1] <- d1 + d2
      ...
      if (k > 1) {
        ...
        for (l in 1:(k - 1)) {
          for (m in (l + 1):k) {
            d1 <-
              sum((protos[l, numvars, drop = FALSE] - protos[m, numvars, drop = FALSE]) ^
                    2) # euclidean for numerics
            d2 <-
              sum(protos[l, catvars, drop = FALSE] != protos[m, catvars, drop = FALSE]) # wtd simple matching for categorics
            if ((d1 + d2) == 0)
              keep.protos[m] <- FALSE
          }
        }
    ...
    if (k == 1) {
      protos <- result2$protos
      k <- result2$k
      clusters <- result2$clusters
      tot.dists <- result2$tot.dists
      moved <- result2$moved #
      dists <- result2$dists
      tot.within <- result2$tot.within
      size <- result2$size
      clusters <- result2$clusters
      iter <- result2$iter
    }
    ...
    if (k > 1) {
      ...
      tot.dists <- result2$tot.dists
      moved <- result2$moved #
      keep.protos <- result2$keep.protos
      ...
      while (iter < iter.max) {
        ...
        for (i in 1:k) {
          ...
          d1 <-
            (x[, numvars, drop = FALSE] - matrix(
              rep(as.numeric(protos[i, numvars, drop = FALSE]), nrows),
              nrow = nrows,
              byrow = T
            )) ^ 2
          if (length(lambda) == 1)
          ...
          d2 <-
            sapply(which(catvars), function(j)
              return(x[, j] != rep(protos[i, j], nrows)))
          if (length(lambda) == 1)
            d2 <- lambda * rowSums(d2)
          if (length(lambda) > 1)
            d2 <- d2 %*% lambda[catvars]
        ...
        size          <- table(clusters)
        min.dists     <-
          apply(cbind(clusters, dists), 1, function(z)
            z[z[1] + 1])
        ...
        if (length(size) < k) {
          ...
          cat("Empty clusters occur. Cluster number reduced to:",
              k,
              "\n\n")
        }
        ...
        for (i in remids) {
          protos[which(remids == i), numvars] <-
            sapply(x[clusters == i, numvars, drop = FALSE], mean)
          protos[which(remids == i), catvars] <-
            sapply(x[clusters == i, catvars, drop = FALSE], function(z)
              levels(z)[which.max(table(z))])
        }
        ...
        if (!all(keep.protos)) {
          ...
          k <- sum(keep.protos)
          cat("Equal prototypes merged. Cluster number reduced to:",
              k,
              "\n\n")
        }
...
predict.kproto <- function(object, newdata, ...) {
  ...
  if (!all(names(object$centers) == names(x)))
    warning("Variable names of newdata do not match!")
  if (ncol(object$centers) != ncol(x))
    stop("Coloumn number of newdata does not match object!")
  if (!all(sapply(object$centers[, numvars, drop = FALSE], is.numeric)))
    stop("Numeric variables of object and newdata do not match!")
  if (!all(sapply(object$centers[, catvars, drop = FALSE], is.factor)))
    stop("Factor variables of object and newdata do not match!")
...
clprofiles <- function(object,
  ...
  if (is.null(vars))
    vars <- 1:ncol(x)
  if (!is.numeric(vars))
    vars <- sapply(vars, function(z)
      return(which(colnames(x) == z)))
  if (length(vars) < 1)
...
lambdaest <-
  function(x,
    ...
    if (!is.data.frame(x))
      stop("x should be a data frame!")
    if (!num.method %in% 1:2)
      stop("Argument 'num.method' must be either 1 or 2!")
    if (!fac.method %in% 1:2)
      stop("Argument 'fac.method' must be either 1 or 2!")
    if (!outtype %in% c("numeric", "vector", "variation"))
      stop("Wrong specificytion of argument 'outtype'!")
    ...
    if (anynum &
        num.method == 1)
      vnum <- sapply(x[, numvars, drop = FALSE], var)
    if (anynum &
        num.method == 2)
      vnum <- sapply(x[, numvars, drop = FALSE], sd)
    ...
    if (anyfact &
      ...
      vcat <-
      sapply(x[, catvars, drop = FALSE], function(z)
        return(1 - max(table(z) / length(z))))
    if (mean(vnum) == 0) {
      warning(
        "All numerical variables have zero variance.\n
    ...
    if (num.method == 2)
      cat("Numeric standard deviations:\n")
    print(vnum)
    if (num.method == 1)
      cat("Average numeric variance:", mean(vnum), "\n\n")
    if (num.method == 2)
      cat("Average numeric standard deviation:", mean(vnum), "\n\n")
    
    cat(paste(
      "Heuristic for categorical variables: (method = ",
      fac.method,
      ") \n",
      sep = ""
    ))
...
#' @export
summary.kproto <- function(object,
                           data = NULL,
                           pct.dig = 3,
                           ...) {
  if (class(object) != "kproto")
    stop("object must be of class kproto!")
  ...
  if (!is.null(data))
    cluster <- predict(object, data)$cluster
  ...
  for (i in 1:ncol(data)) {
    ...
    if (catvars[i])
      res[[i]] <-
        round(prop.table(table(cluster, data[, i]), 1), digits = pct.dig)
    print(res[[i]])
    cat("\n-----------------------------------------------------------------\n")
  }
